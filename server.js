const express = require('express');
const http = require('http');
const socketIO = require('socket.io');
const path = require('path');
const os = require('os');
const QRCode = require('qrcode');
const fs = require('fs');

// Initialize our express app
const app = express();
const server = http.createServer(app);
const io = socketIO(server);

// Serve static files from the public directory
app.use(express.static(path.join(__dirname, 'public')));

// Serve markers directory for easy access to marker generators
app.use('/markers', express.static(path.join(__dirname, 'markers')));

// Game configuration
const gameConfig = {
    mode: 'freeplay', // 'freeplay', 'team', 'countdown'
    respawnTime: 5, // seconds
    gameTime: 300, // seconds (5 minutes)
    teams: ['red', 'blue']
};

// Game state
let gameActive = false;
let gameTimer = null;
let timeRemaining = gameConfig.gameTime;

// Track connected players
const players = new Map();
const teams = {
    red: { score: 0, players: new Set() },
    blue: { score: 0, players: new Set() }
};

// Socket.io connection handling
io.on('connection', (socket) => {
  console.log('New client connected:', socket.id);
  
  // Add new player
  players.set(socket.id, {
    id: socket.id,
    name: `Player ${players.size + 1}`,
    score: 0,
    lastHit: null,
    team: null,
    isActive: true,
    respawnTime: 0
  });
  
  // Send current game state to the new player
  socket.emit('gameState', {
    mode: gameConfig.mode,
    active: gameActive,
    timeRemaining: timeRemaining,
    teams: {
      red: { score: teams.red.score, playerCount: teams.red.players.size },
      blue: { score: teams.blue.score, playerCount: teams.blue.players.size }
    }
  });
  
  // Broadcast updated player list
  io.emit('playerList', Array.from(players.values()));
  
  // Listen for name changes
  socket.on('setName', (name) => {
    const player = players.get(socket.id);
    if (player) {
      player.name = name;
      io.emit('playerList', Array.from(players.values()));
    }
  });
  
  // Listen for team join requests
  socket.on('joinTeam', (teamName) => {
    if (!teams[teamName]) return;
    
    const player = players.get(socket.id);
    if (player) {
      // Remove from previous team if any
      if (player.team && teams[player.team]) {
        teams[player.team].players.delete(socket.id);
      }
      
      // Add to new team
      player.team = teamName;
      teams[teamName].players.add(socket.id);
      
      // Notify clients
      socket.emit('teamAssigned', teamName);
      io.emit('playerList', Array.from(players.values()));
      io.emit('teamUpdate', {
        red: { score: teams.red.score, playerCount: teams.red.players.size },
        blue: { score: teams.blue.score, playerCount: teams.blue.players.size }
      });
    }
  });
  
  // Listen for hit events
  socket.on('hit', (targetId) => {
    const target = players.get(targetId);
    const shooter = players.get(socket.id);
    
    if (!target || !shooter) return;
    if (targetId === socket.id) return; // Prevent self-shooting
    if (!target.isActive || !shooter.isActive) return; // Only active players can hit/be hit
    
    // In team mode, prevent friendly fire
    if (gameConfig.mode === 'team' && target.team === shooter.team) return;
    
    // Add score to shooter
    shooter.score += 1;
    target.lastHit = shooter.id;
    
    // Add team score in team mode
    if (gameConfig.mode === 'team' && shooter.team) {
      teams[shooter.team].score += 1;
    }
    
    // Handle respawn in appropriate game modes
    if (gameConfig.mode !== 'freeplay') {
      target.isActive = false;
      target.respawnTime = gameConfig.respawnTime;
      
      // Set a timer to respawn the player
      setTimeout(() => {
        if (players.has(targetId)) {
          const targetPlayer = players.get(targetId);
          targetPlayer.isActive = true;
          targetPlayer.respawnTime = 0;
          io.to(targetId).emit('respawned');
          io.emit('playerList', Array.from(players.values()));
        }
      }, gameConfig.respawnTime * 1000);
    }
    
    // Notify all clients
    io.emit('playerHit', {
      shooter: shooter.id,
      target: targetId,
      shooterTeam: shooter.team,
      targetTeam: target.team
    });
    
    // Update player list
    io.emit('playerList', Array.from(players.values()));
    
    // Update team scores
    if (gameConfig.mode === 'team') {
      io.emit('teamUpdate', {
        red: { score: teams.red.score, playerCount: teams.red.players.size },
        blue: { score: teams.blue.score, playerCount: teams.blue.players.size }
      });
    }
  });
  
  // Admin commands (would implement authentication in production)
  socket.on('startGame', (options = {}) => {
    // Update game config with any provided options
    if (options.mode) gameConfig.mode = options.mode;
    if (options.respawnTime) gameConfig.respawnTime = options.respawnTime;
    if (options.gameTime) gameConfig.gameTime = options.gameTime;
    
    // Reset game state
    timeRemaining = gameConfig.gameTime;
    gameActive = true;
    teams.red.score = 0;
    teams.blue.score = 0;
    
    // Start game timer
    if (gameTimer) clearInterval(gameTimer);
    gameTimer = setInterval(() => {
      timeRemaining--;
      
      // Broadcast time update every 5 seconds
      if (timeRemaining % 5 === 0) {
        io.emit('timeUpdate', timeRemaining);
      }
      
      // End game when timer reaches zero
      if (timeRemaining <= 0) {
        endGame();
      }
    }, 1000);
    
    // Notify clients
    io.emit('gameStarted', {
      mode: gameConfig.mode,
      timeRemaining: timeRemaining
    });
  });
  
  socket.on('endGame', () => {
    endGame();
  });
  
  // Reset scores for all players
  socket.on('resetScores', () => {
    // Reset all player scores
    players.forEach(player => {
      player.score = 0;
    });
    
    // Reset team scores
    teams.red.score = 0;
    teams.blue.score = 0;
    
    // Notify all clients
    io.emit('playerList', Array.from(players.values()));
    io.emit('teamUpdate', {
      red: { score: teams.red.score, playerCount: teams.red.players.size },
      blue: { score: teams.blue.score, playerCount: teams.blue.players.size }
    });
    
    // Confirm reset
    io.emit('scoresReset');
  });
  
  // Handle disconnect
  socket.on('disconnect', () => {
    console.log('Client disconnected:', socket.id);
    
    // Remove from team if assigned
    const player = players.get(socket.id);
    if (player && player.team && teams[player.team]) {
      teams[player.team].players.delete(socket.id);
    }
    
    players.delete(socket.id);
    io.emit('playerList', Array.from(players.values()));
    
    // Update team counts
    io.emit('teamUpdate', {
      red: { score: teams.red.score, playerCount: teams.red.players.size },
      blue: { score: teams.blue.score, playerCount: teams.blue.players.size }
    });
  });
});

// End the current game
function endGame() {
  gameActive = false;
  
  if (gameTimer) {
    clearInterval(gameTimer);
    gameTimer = null;
  }
  
  // Calculate results
  let winner = null;
  let topPlayers = [];
  
  if (gameConfig.mode === 'team') {
    winner = teams.red.score > teams.blue.score ? 'red' : 
            teams.blue.score > teams.red.score ? 'blue' : 'tie';
  } else {
    // Get top 3 players
    topPlayers = Array.from(players.values())
      .sort((a, b) => b.score - a.score)
      .slice(0, 3);
  }
  
  // Notify clients
  io.emit('gameEnded', {
    mode: gameConfig.mode,
    winner: winner,
    topPlayers: topPlayers,
    teamScores: {
      red: teams.red.score,
      blue: teams.blue.score
    }
  });
}

// Find the server's local IP address
function getLocalIP() {
  const interfaces = os.networkInterfaces();
  for (const iface in interfaces) {
    for (const details of interfaces[iface]) {
      // Skip over non-IPv4 and internal (loopback) addresses
      if (details.family === 'IPv4' && !details.internal) {
        return details.address;
      }
    }
  }
  return '127.0.0.1'; // Default to localhost if no external IP found
}

// Generate QR code for easy access
function generateQRCode(url) {
  const qrPath = path.join(__dirname, 'public', 'qr.png');
  
  QRCode.toFile(qrPath, url, {
    errorCorrectionLevel: 'H',
    margin: 1,
    scale: 8,
    color: {
      dark: '#000000',
      light: '#ffffff'
    }
  }, (err) => {
    if (err) {
      console.error('Error generating QR code:', err);
    } else {
      console.log('QR code generated successfully at /qr.png');
    }
  });

  // Also generate the QR code as a data URL for console display
  QRCode.toString(url, {
    type: 'terminal',
    errorCorrectionLevel: 'L',
    small: true
  }, (err, qrString) => {
    if (!err) {
      console.log('\nScan this QR code to join the game:');
      console.log(qrString);
    }
  });
}

// Redirect /join to the join.html page
app.get('/join', (req, res) => {
  res.sendFile(path.join(__dirname, 'public', 'join.html'));
});

// Start the server
const PORT = process.env.PORT || 3000;
server.listen(PORT, () => {
  const ip = getLocalIP();
  const url = `http://${ip}:${PORT}/join`;
  
  console.log(`Server running on port ${PORT}`);
  console.log(`Local IP address: ${ip}`);
  console.log(`\nShare this URL with players: ${url}`);
  
  // Generate QR code for the URL
  generateQRCode(url);
  
  // Also create a text file with the URL for easy sharing
  fs.writeFileSync(path.join(__dirname, 'game-url.txt'), url);
});
